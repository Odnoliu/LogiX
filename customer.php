...
<body class="bg-gradient-to-br from-blue-50 to-blue-100 min-h-screen flex flex-col">
    ...
    <template id="order-card-template">
      <article class="bg-white rounded-2xl shadow-lg overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl animate-fadeInUp">
        ...
        <div class="p-5 flex flex-col gap-2">
          ...
          <!-- Inline chọn giao hàng -->
          <div class="mt-3 grid gap-2">
            <label class="text-sm font-medium text-gray-700">Chọn thời gian giao</label>
            <input type="time" class="time-input w-full px-3 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-300">
  ...
  (() => {
    ...
    const lockers = [
      { id: "locker-1", label: "Smart Locker 1 - Vincom Xuân Khánh" },
      { id: "locker-2", label: "Smart Locker 2 - Lotte Mart Cần Thơ" },
      { id: "locker-3", label: "Smart Locker 3 - Coopmart" },
      { id: "locker-4", label: "Smart Locker 4 - BigC" },
      { id: "locker-5", label: "Smart Locker 5 - Sense City" }
    ];
    ...
    // Dữ liệu ảo đơn hàng
    const fakeOrders = [
      { DH_Ma: "DH001", KH_Ten: "Nguyễn Văn A", DH_SoLuongKIH: 2, DH_TongTien: 500000, TT_Ma: "TT001" },
      { DH_Ma: "DH002", KH_Ten: "Trần Thị B", DH_SoLuongKIH: 1, DH_TongTien: 200000, TT_Ma: "TT002" },
      { DH_Ma: "DH003", KH_Ten: "Lê Văn C", DH_SoLuongKIH: 3, DH_TongTien: 750000, TT_Ma: "TT001" },
      { DH_Ma: "DH004", KH_Ten: "Phạm Thị D", DH_SoLuongKIH: 5, DH_TongTien: 1500000, TT_Ma: "TT003" },
      { DH_Ma: "DH005", KH_Ten: "Huỳnh Văn E", DH_SoLuongKIH: 4, DH_TongTien: 990000, TT_Ma: "TT001" }
    ];

    const statusConfigs = {
      'TT001': { label: 'Đang Chờ Xử Lý', btnText: 'Xác nhận giao hàng', enabled: true },
      'TT002': { label: 'Đang Giao Hàng', btnText: 'Đang giao', enabled: false },
      'TT003': { label: 'Đã Giao Hàng', btnText: 'Đã giao', enabled: false }
    };
    ...
    function renderOrders(orders) {
      ...
      orders.forEach((order, idx) => {
        ...
        const statusConfig = statusConfigs[order.TT_Ma] || { label: "Không xác định", btnText: "-", enabled: false };
        ...
        confirmBtn.addEventListener('click', () => {
          ...
          showToast(`Đơn ${order.DH_Ma} đã được xác nhận giao lúc ${timeInput.value}.`, "success");
